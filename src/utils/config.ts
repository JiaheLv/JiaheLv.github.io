...
export function getSiteConfig(): DynamicSiteConfig {
  ...
  const githubRepoName = process.env.GITHUB_REPO_NAME || 'JiaheLv.github.io'; // 默认改为你的 GitHub 个人站仓库名
  const githubActor = process.env.GITHUB_ACTOR || 'JiaheLv'; // 默认改为你的 GitHub 用户名
  ...
  switch (deployEnv) {
    ...
    case 'MAIN_CLOUDFLARE':
      dynamicUrl = originalSiteConfigFromFile.site.url; // 保持从 site.json 读取自定义域名
      dynamicBase = '/';
      break;
    default: // LOCAL or other environments
      // 对于本地开发，使用 site.json 中的 URL 或本地默认地址
